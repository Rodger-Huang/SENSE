...
from packnet_sfm.networks.edge import vgg16_c
...
class BDCN(nn.Module):
    ...
    def forward(self, x):
        ...
        sum1 = self.conv1_1_down(self.msblock1_1(features[0])) + \
                self.conv1_2_down(self.msblock1_2(features[1])) # (B, 21, H, W)
        s1 = self.score_dsn1(sum1) # (B, 1, H, W)
        s11 = self.score_dsn1_1(sum1) # (B, 1, H, W)

        sum2 = self.conv2_1_down(self.msblock2_1(features[2])) + \
            self.conv2_2_down(self.msblock2_2(features[3])) # (B, 21, H//2, W//2)
        s2 = self.score_dsn2(sum2) # (B, 1, H//2, W//2)
        s21 = self.score_dsn2_1(sum2) # (B, 1, H//2, W//2)
        s2 = self.upsample_2(s2)
        s21 = self.upsample_2(s21)
        s2 = crop(s2, x, 1, 1) # (B, 1, H, W)
        s21 = crop(s21, x, 1, 1) # (B, 1, H, W)

        sum3 = self.conv3_1_down(self.msblock3_1(features[4])) + \
            self.conv3_2_down(self.msblock3_2(features[5])) + \
            self.conv3_3_down(self.msblock3_3(features[6])) # (B, 21, H//4, W//4)
        s3 = self.score_dsn3(sum3)
        s3 =self.upsample_4(s3)
        s3 = crop(s3, x, 2, 2) # (B, 1, H, W)
        s31 = self.score_dsn3_1(sum3)
        s31 =self.upsample_4(s31)
        s31 = crop(s31, x, 2, 2) # (B, 1, H, W)

        sum4 = self.conv4_1_down(self.msblock4_1(features[7])) + \
            self.conv4_2_down(self.msblock4_2(features[8])) + \
            self.conv4_3_down(self.msblock4_3(features[9])) # (B, 21, H//8, W//8)
        ...
        return [p1_1, p2_1, p3_1, p4_1, p5_1, p1_2, p2_2, p3_2, p4_2, p5_2, fuse], [sum1, sum2, sum3, sum4, sum5]
