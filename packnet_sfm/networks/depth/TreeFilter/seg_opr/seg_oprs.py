...
class ConvGNELU(nn.Module):
    def __init__(self, in_planes, out_planes, ksize, stride, pad, dilation=1,
                 groups=1, has_bn=True, norm_layer=nn.BatchNorm2d, bn_eps=1e-5,
                 has_relu=True, inplace=True, has_bias=False):
        super(ConvGNELU, self).__init__()
        self.conv = nn.Conv2d(in_planes, out_planes, kernel_size=ksize,
                              stride=stride, padding=pad,
                              dilation=dilation, groups=groups, bias=has_bias)
        self.has_bn = has_bn
        if self.has_bn:
            self.normalize = nn.GroupNorm(16, out_planes)
        self.has_relu = has_relu
        if self.has_relu:
            self.nonlin = nn.ELU(inplace=inplace)
...
class RefineResidual(nn.Module):
    ...
    def forward(self, x):
        ...
        t = self.conv_refine(t)
        if self.has_relu:
            return self.nonlin(self.normalize(t + x))
        return t + x
