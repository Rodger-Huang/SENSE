...
from packnet_sfm.networks.depth.DeformableTransformerResNet.resnet.resnet_encoder import ResnetEncoder
from packnet_sfm.networks.depth.DeformableTransformerResNet.resnet.depth_decoder import DepthDecoder
from packnet_sfm.networks.depth.DeformableTransformerResNet.resnet.layers import disp_to_depth
...
class TransformerResNet(nn.Module):
    ...
    def __init__(self, version=None, **kwargs):
        ...
        self.decoder = DepthDecoder(num_ch_enc=self.encoder.num_ch_enc, scales=range(3))
    ...
    def forward(self, x):
        ...
        x = self.encoder(x)
        x = self.decoder(x)
        disps = [x[('disp', i)] for i in range(3)]
