...
from packnet_sfm.networks.depth.DeformableTransformerResNet.ops.modules import MSDeformAttn
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        # self.patch_to_embedding_pos = nn.Linear(d_model * p, d_model)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, tgt):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            _, _, tgt_h, tgt_w = tgt.shape
            tgt_spatial_shape = (tgt_h, tgt_w)
            tgt_spatial_shapes.append(tgt_spatial_shape)
        ...
        query_embed = query_embed.flatten(2).transpose(1, 2)
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points,
                memory,
                src_spatical_shapes, src_level_start_index,
                tgt_spatial_shapes, tgt_level_start_index,
                src_valid_ratios,
        ...
        intermediate = []
        # 编码每个解码器
        tgt_reference_points_input = self.get_reference_points(tgt_spatial_shapes, src_valid_ratios, device=memory.device)
        reference_points_input = tgt_reference_points_input
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output, tgt_reference_points_input, reference_points_input, memory, 
                           src_spatical_shapes, src_level_start_index, 
                           tgt_spatial_shapes, tgt_level_start_index,
                           src_padding_mask,
                           tgt_mask=tgt_mask,
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, tgt_reference_points_input, reference_points, memory,
                     src_spatical_shapes, level_start_index, 
                     tgt_spatical_shapes, tgt_level_start_index,
                     src_padding_mask=None,
                     tgt_mask: Optional[Tensor] = None,
                     memory_mask: Optional[Tensor] = None,
                     tgt_key_padding_mask: Optional[Tensor] = None,
        ...
        tgt2 = self.self_attn(q, 
                              tgt_reference_points_input,
                              tgt,
                              tgt_spatical_shapes, tgt_level_start_index, None
                              )
    ...
    def forward(self, tgt, tgt_reference_points_input, reference_points, 
                memory,
                src_spatical_shapes, level_start_index,
                tgt_spatical_shapes, tgt_level_start_index,
                src_padding_mask=None,
        ...
        return self.forward_post(tgt, tgt_reference_points_input, reference_points, memory, 
                                 src_spatical_shapes, level_start_index, 
                                 tgt_spatical_shapes, tgt_level_start_index,
                                 src_padding_mask, 
...
def build_deforamble_transformer(hidden_dim, args):
