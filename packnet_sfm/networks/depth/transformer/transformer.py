...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        # p = 16
        # src = rearrange(src, 'b c (h p1) (w p2) -> (h w) b (p1 p2 c)', p1 = p, p2 = p)
        # pos_embed = rearrange(pos_embed, 'b c (h p1) (w p2) -> (h w) b (p1 p2 c)', p1 = p, p2 = p)
        # # src = self.patch_to_embedding_src(src)
        # # pos_embed = self.patch_to_embedding_pos(pos_embed)
        # mask = rearrange(mask, 'b (h p1) (w p2) -> b (h w) (p1 p2)', p1 = p, p2 = p)
        # mask = mask[:, :, 0]
        ...
        # memory = rearrange(memory, '(h w) b (p1 p2 c) -> b c (h p1) (w p2)', h = 12, w = 40, p1 = p, p2 = p)

        # p = 16
        # hs = rearrange(hs, 'm (h w) b (p1 p2 c) -> m b c (h p1) (w p2)', h = 12, w = 40, p1 = p, p2 = p)

        hs = hs.permute(0, 2, 3, 1).view(6, bs, c, 12, 40)
