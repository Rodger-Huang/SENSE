...
from packnet_sfm.networks.layers.resnet.HR_Depth_Decoder import HRDepthDecoder
...
class HRDepthResNet(nn.Module):
    ...
    def __init__(self, version=None, **kwargs):
        ...
        self.decoder = HRDepthDecoder(num_ch_enc=self.encoder.num_ch_enc)
    ...
    def forward(self, x):
        ...
        x = self.encoder(x)
        x = self.decoder(x)
        disps = [x[("disparity", "Scale0")], x[("disparity", "Scale1")], x[("disparity", "Scale2")], x[("disparity", "Scale3")]]
