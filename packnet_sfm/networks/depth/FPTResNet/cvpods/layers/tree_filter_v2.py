...
from .tree_filter_core import RandomSpanningTree, MinimumSpanningTree
from .tree_filter_core import TreeFilter2D


class TreeFilterV2(nn.Module):
    def __init__(self, guide_channels, in_channels, guide_embed_channels, in_embed_channels, num_groups=1, eps=1e-8):
        super(TreeFilterV2, self).__init__()
        ''' Hyper Parameters '''
        self.eps            = eps
        self.guide_channels = guide_channels
        self.in_channels    = in_channels
        self.guide_embed_channels = guide_embed_channels
        self.in_embed_channels = in_embed_channels
        self.num_groups     = num_groups

        ''' Embedding Layers '''
        self.embed_layer = nn.Conv2d(in_channels, in_embed_channels, kernel_size=1, bias=False)
        self.conf_layer  = nn.Conv2d(in_channels, num_groups, kernel_size=1, bias=False)
        self.guide_layer = nn.Conv2d(guide_channels, guide_embed_channels, kernel_size=1, bias=False)
    ...
    def reset_parameter(self):
        ...
        # nn.init.normal_(self.beta, std=0.001)
