...
class AxialAttention(nn.Module):
    def __init__(self, qk_in_planes, v_in_planes, out_planes, groups=8, kernel_size=56,
                 stride=1, bias=False, width=False):
        assert (v_in_planes % groups == 0) and (out_planes % groups == 0)
        super(AxialAttention, self).__init__()
        self.qk_in_planes = qk_in_planes
        self.v_in_planes = v_in_planes
        ...
        self.v_transform = v_transform(v_in_planes, int(out_planes), kernel_size=1, stride=1,
                                           padding=0, bias=False)
        self.bn_v = nn.BatchNorm1d(int(out_planes))
        self.qk_transform = qk_transform(qk_in_planes, out_planes, kernel_size=1, stride=1,
                                           padding=0, bias=False)
        self.bn_qk = nn.BatchNorm1d(out_planes)
    ...
    def reset_parameters(self):
        self.v_transform.weight.data.normal_(0, math.sqrt(1. / self.v_in_planes))
        self.qk_transform.weight.data.normal_(0, math.sqrt(1. / self.qk_in_planes))
    ...
    def forward(self, x, qk):
        ...
        B, W, C, H = qk.shape
        qk = qk.contiguous().view(B * W, C, H)

        # Transformations
        v = self.bn_v(self.v_transform(x))
        v = v.reshape(B * W, self.groups, self.group_planes, H)

        qk = self.bn_qk(self.qk_transform(qk))
        q, k = torch.split(qk.reshape(B * W, self.groups, int(self.group_planes), H), [self.group_planes // 2, self.group_planes // 2], dim=2)
        ...
        stacked_similarity = self.bn_similarity(stacked_similarity).view(B * W, 3, self.groups, H, H).sum(dim=1)
        #stacked_similarity = self.bn_qr(qr) + self.bn_kr(kr) + self.bn_qk(qk)
        # (B, groups, H, H, W)
        similarity = F.softmax(stacked_similarity, dim=3)
        sv = torch.einsum('bgij, bgcj -> bgci', similarity, v)
        sve = torch.einsum('bgij, cij -> bgci', similarity, v_embedding)
        stacked_output = torch.cat([sv, sve], dim=-1).view(B * W, self.out_planes * 2, H)
        output = self.bn_output(stacked_output).view(B, W, self.out_planes, 2, H).sum(dim=-2)
