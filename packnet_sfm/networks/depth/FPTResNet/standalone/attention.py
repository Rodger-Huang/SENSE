...
class AttentionConv(nn.Module):
    ...
    def forward(self, x, qk):
        ...
        padded_x = F.pad(x, [self.padding, self.padding, self.padding, self.padding]) # (B, C, , )
        padded_qk = F.pad(qk, [self.padding, self.padding, self.padding, self.padding]) # (B, C, , )
        q_out = self.query_conv(qk)
        k_out = self.key_conv(padded_qk)
        v_out = self.value_conv(padded_x)

        k_out = k_out.unfold(2, self.kernel_size, self.stride).unfold(3, self.kernel_size, self.stride)
        v_out = v_out.unfold(2, self.kernel_size, self.stride).unfold(3, self.kernel_size, self.stride)

        k_out_h, k_out_w = k_out.split((self.out_channels // 2, self.out_channels // 2), dim=1)
