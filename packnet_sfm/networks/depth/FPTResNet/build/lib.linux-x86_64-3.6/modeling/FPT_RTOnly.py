...
class FPT_RTOnly(nn.Module):
    def __init__(self, feature_dim, with_norm='none', upsample_method='bilinear'):
        ...
        self.rt_p5_p4 = RenderTrans(channels_high=feature_dim, channels_low=feature_dim, level=1, upsample=False)
        self.rt_p5_p3 = RenderTrans(channels_high=feature_dim, channels_low=feature_dim, level=2, upsample=False)
        self.rt_p5_p2 = RenderTrans(channels_high=feature_dim, channels_low=feature_dim, level=3, upsample=False)
        self.rt_p4_p3 = RenderTrans(channels_high=feature_dim, channels_low=feature_dim, level=1, upsample=False)
        self.rt_p4_p2 = RenderTrans(channels_high=feature_dim, channels_low=feature_dim, level=2, upsample=False)
        self.rt_p3_p2 = RenderTrans(channels_high=feature_dim, channels_low=feature_dim, level=1, upsample=False)
        ...
        if with_norm != 'none':
            ...
            self.fpt_p5 = nn.Conv2d(feature_dim*4, feature_dim, 3, padding=1)
            self.fpt_p4 = nn.Conv2d(feature_dim*3, feature_dim, 3, padding=1)
            self.fpt_p3 = nn.Conv2d(feature_dim*2, feature_dim, 3, padding=1)
            self.fpt_p2 = nn.Conv2d(feature_dim*1, feature_dim, 3, padding=1)
    ...
    def forward(self, res2, res3, res4, res5):
        ...
        fpt_p5_out = torch.cat((self.rt_p5_p4(fpn_p5_1, fpn_p4_1), 
            self.rt_p5_p3(fpn_p5_1,fpn_p3_1), self.rt_p5_p2(fpn_p5_1,fpn_p2_1), fpn_p5_1), 1)
        fpt_p4_out = torch.cat((self.rt_p4_p3(fpn_p4_1, fpn_p3_1), 
            self.rt_p4_p2(fpn_p4_1,fpn_p2_1), fpn_p4_1), 1)
        fpt_p3_out = torch.cat((self.rt_p3_p2(fpn_p3_1, fpn_p2_1), fpn_p3_1), 1)
        fpt_p2_out = torch.cat((fpn_p2_1, ), 1)
