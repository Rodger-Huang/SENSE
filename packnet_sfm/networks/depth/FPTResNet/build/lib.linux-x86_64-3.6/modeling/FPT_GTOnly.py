...
class FPT_GTOnly(nn.Module):
    def __init__(self, feature_dim, with_norm='none', upsample_method='bilinear'):
        ...
        if with_norm != 'none':
            ...
            self.fpt_p5 = nn.Conv2d(feature_dim*1, feature_dim, 3, padding=1)
            self.fpt_p4 = nn.Conv2d(feature_dim*2, feature_dim, 3, padding=1)
            self.fpt_p3 = nn.Conv2d(feature_dim*3, feature_dim, 3, padding=1)
    ...
    def forward(self, res2, res3, res4, res5):
        ...
        fpt_p5_out = torch.cat((fpn_p5_1, ), 1)
        fpt_p4_out = torch.cat((self.gt_p4_p5(fpn_p4_1,fpn_p5_1), fpn_p4_1), 1)
        fpt_p3_out = torch.cat((self.gt_p3_p4(fpn_p3_1,fpn_p4_1), self.gt_p3_p5(fpn_p3_1,fpn_p5_1), fpn_p3_1), 1)
