...
from packnet_sfm.networks.depth.hrt.modules.bottleneck_block import Bottleneck, BottleneckDWP
from packnet_sfm.networks.depth.hrt.modules.transformer_block import GeneralTransformerBlock

from functools import partial
from packnet_sfm.networks.depth.hrt.config import parse_option
from packnet_sfm.networks.depth.hrt.SwinTransformer import SwinTransformer

blocks_dict = {
    "BOTTLENECK": Bottleneck,
    # "TRANSFORMER_BLOCK": GeneralTransformerBlock,
    "TRANSFORMER_BLOCK": SwinTransformer,
}
...
class HRSwinTransformerModule(nn.Module):
    def __init__(
        self,
        num_branches,
        blocks,
        ...
        """
        super(HRSwinTransformerModule, self).__init__()
    ...
    def _make_one_branch(
        ...
        layers.append(
            block(
                self.num_inchannels[branch_index],
                embed_dim=num_channels[branch_index],
...
class HRSwinTransformer(nn.Module):
    def __init__(self, version, scales, **kwargs):
        super(HRSwinTransformer, self).__init__()
        
        _, cfg = parse_option(version)
        cfg = cfg.MODEL.HRT
        ...
        for i in range(self.scales):
            self.last_layer.append(nn.Sequential(
                nn.Conv2d(
                    ...
                    stride=1,
                    padding=0),
                nn.BatchNorm2d(pre_stage_channels[i], momentum=BN_MOMENTUM),
    ...
    def _make_layer(
        ...
        if isinstance(block, SwinTransformer):
            layers.append(
                block(
                    inplanes,
                    planes,
    ...
    def forward(self, x):
        ...
        # Upsampling
        for i in range(self.scales):
            x_dict[i] = F.upsample(y_list[i], scale_factor=4, mode='bilinear')
        
        disps = []
        for i in range(self.scales):
            disps.append(self.last_layer[i](x_dict[i]))
