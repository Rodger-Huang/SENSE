...
def transformerFwd(U,
    ...
    def _interpolate(im, x, y, out_size):
        ...
        zerof = torch.zeros_like(wa) # zeros_like()：生成和括号内变量维度维度一致的全是零的内容
        # where(condition, x, y)
        # 针对于x而言，如果其中的每个元素都满足condition，就返回x的值；
        # 如果不满足condition，就将y对应位置的元素或者y的值(如果y为单元素tensor的话)替换x的值，最后返回结果
        # eq()
        # 相同位置值相同则返回对应的True,返回的是一个列表
        wa = torch.where(
    ...
    def _transform(flo, input_dim, out_size):
        ...
        x_s = x_s.to(flo.get_device()).unsqueeze(0) # torch.unsqueeze() 扩展维度，返回一个新的张量，对输入的既定位置插入维度 1
...
class Model_flow(nn.Module):
    def __init__(self, mode=None, dataset=None, num_scales=3, h_flow_consist_alpha=3.0, h_flow_consist_beta=0.05, **kwargs):
        super(Model_flow, self).__init__()
        self.fpyramid = FeaturePyramid()
        self.pwc_model = PWC_tf()
        if mode == 'depth' or mode == 'flowposenet':
            # Stage 2 training
        ...
        self.flow_consist_alpha = h_flow_consist_alpha # alpha = 3.0
        self.flow_consist_beta = h_flow_consist_beta # beta = 0.05

    def get_occlusion_mask_from_flow(self, tensor_size, flow):
        """[summary]

        Args:
            tensor_size ([type]): [batch_size, 1, ]
            flow ([type]): [description]

        Returns:
            occ_mask ([type]): occlusion mask
        """
    ...
    def get_visible_masks(self, optical_flows, optical_flows_rev):
        ...
        Args:
            optical_flows ([type]): [description]
            optical_flows_rev ([type]): [description]

        Returns:
            img1_visible_masks ([type]): 用反向光流计算image1的遮挡情况
            img2_visible_masks ([type]): 用前向光流计算image2的遮挡情况
        """
        # get occlusion masks
    ...
    def get_consistent_masks(self, optical_flows, optical_flows_rev):
        ...
        for s, (optical_flow, optical_flow_rev) in enumerate(zip(optical_flows, optical_flows_rev)):
            ...
            with torch.no_grad():
                # delta(pt): ||Delta ffullt-s(pt)||2 < max{beta||ffullt-s(pt)||2, alpha}
    ...
    def forward(self, image, ref_image, output_flow=False, use_flow_loss=True):
        assert (image.shape[1] == 3)
        img_h, img_w = image.shape[2], image.shape[3] 
        img1, img2 = image, ref_image
        ...
        # get the optical flows and reverse optical flows for each pair of adjacent images
        feature_list_1, feature_list_2 = self.fpyramid(img1), self.fpyramid(img2)
        optical_flows = self.pwc_model(feature_list_1, feature_list_2, [img_h, img_w]) # 4 * [B, 2, H, W]
        optical_flows_rev = self.pwc_model(feature_list_2, feature_list_1, [img_h, img_w])
        
        # get occlusion masks: Mo
        img2_visible_masks, img1_visible_masks = self.get_visible_masks(optical_flows, optical_flows_rev)
        # get forward-backforward consistent masks: Ms
        img2_consis_masks, img1_consis_masks, fwd_flow_diff_pyramid, bwd_flow_diff_pyramid = self.get_consistent_masks(optical_flows, optical_flows_rev)
        # get final valid masks 
        img2_valid_masks, img1_valid_masks = [], []
        ...
        loss_pack['loss_flow_consis'] = torch.zeros([2]).to(img1.get_device()).requires_grad_()
        if output_flow:
            return loss_pack, optical_flows[0], optical_flows_rev[0], img1_valid_masks[0], img2_valid_masks[0], \
                fwd_flow_diff_pyramid[0], bwd_flow_diff_pyramid[0], img1_warped_pyramid[0], img1_visible_masks[0], img1_consis_masks[0]
        else:
            return loss_pack
