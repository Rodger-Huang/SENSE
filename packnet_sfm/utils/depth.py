...
def write_depth(filename, depth, intrinsics=None, 
        pseudo_mean_mask=None, pseudo_otsu_mask=None,
        GMM_prob=None):
    """
    ...
    if is_tensor(pseudo_mean_mask):
        pseudo_mean_mask = pseudo_mean_mask.detach().squeeze().cpu()
    if is_tensor(pseudo_otsu_mask):
        pseudo_otsu_mask = pseudo_otsu_mask.detach().squeeze().cpu()
    if is_tensor(GMM_prob):
        GMM_prob = GMM_prob.detach().squeeze().cpu()
    # If we are saving as a .npz
    if filename.endswith('.npz'):
        np.savez_compressed(filename, depth=depth, intrinsics=intrinsics, 
            pseudo_mean_mask=pseudo_mean_mask, pseudo_otsu_mask=pseudo_otsu_mask,
            GMM_prob=GMM_prob)
...
def viz_inv_depth(inv_depth, normalizer=None, percentile=95,
    ...
    if is_tensor(inv_depth):
        ...
        if len(inv_depth.shape) == 3:
            inv_depth = inv_depth.squeeze(0) # [H, W]
        inv_depth = inv_depth.detach().cpu().numpy() # [H, W]
...
def compute_depth_metrics(config, gt, pred, use_gt_scale=True):
    ...
    for pred_i, gt_i in zip(pred, gt):
        ...
        gt_i_map = torch.zeros_like(gt_i)
        gt_i_map[valid==True] = gt_i[valid==True]
        pred_i_map = torch.zeros_like(pred_i)
        far_close_map = torch.zeros_like(pred_i)
        ...
        pred_i_map[valid==True] = pred_i
        base_map = pred_i_map - gt_i_map
        far_close_map[base_map < 0.] = 100
        far_close_map[base_map > 0.] = 50
        error_map = torch.abs(base_map)
    ...
    return torch.tensor([metric / batch_size for metric in
        [abs_rel, sq_rel, rmse, rmse_log, a1, a2, a3]]).type_as(gt), \
            error_map, gt_i_map, far_close_map
