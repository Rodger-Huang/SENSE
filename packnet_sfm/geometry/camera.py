...
from packnet_sfm.utils.image import image_grid, meshgrid
from packnet_sfm.networks.opticalFlow.model_flow import transformerFwd
...
########################################################################################################################
    ...
    def reconstruct(self, depth, frame='w'):
        ...
        # If in world frame of reference
            ...
            return self.Twc @ Xc # In python 3.5, the @ operator was introduced for matrix multiplication
    ...
    def project(self, X, frame='w'):
        ...
        # X2 = X / Z
        # Y2 = Y / Z
        # img2 = torch.stack([X2, Y2], dim=-1).view(B, H, W, 2).permute(0, 3, 1, 2)
        # xs, ys = meshgrid(B, H, W, X.dtype, X.device, normalized=False)
        # grid = torch.stack([xs, ys], dim=1)
        # rigid_flow = img2 - grid
        # shape = [B, 1, int(H), int(W)]
        # mask = torch.ones(shape).to(rigid_flow.get_device())
        # h, w = mask.shape[2], mask.shape[3]
        # occ_mask = transformerFwd(mask.permute(0,2,3,1), rigid_flow.permute(0,2,3,1), out_size=[h, w]).permute(0,3,1,2)
        # with torch.no_grad():
        ...
        return torch.stack([Xnorm, Ynorm], dim=-1).view(B, H, W, 2) # , occ_mask
