...
from packnet_sfm.utils.util import get_colorization_data, lab2rgb
...
class KITTIDataset(Dataset):
    ...
    def __init__(self, root_dir, file_list, train=True,
                 ...
                 with_FDA_target=False, with_pseudo_inv_depth=False,
                 back_context=0, forward_context=0, strides=(1,), pseudo_label_path=''):
        # Assertions
        ...
        self.with_FDA_target = with_FDA_target
        self.with_pseudo_inv_depth = with_pseudo_inv_depth

        self._cache = {} # 记录每一个文件夹下图片总数
        self.pose_cache = {}
        self.oxts_cache = {}
...
########################################################################################################################
    ...
    def __getitem__(self, idx):
        ...
        if self.with_pseudo_inv_depth:
            pseudo = np.load('{}/{}_depth.npz'.format(self.pseudo_label_path, sample['filename']))
            sample.update({
                'pseudo_inv_depth': pseudo['depth'],
                'GMM_prob': pseudo['GMM_prob'],
            })
