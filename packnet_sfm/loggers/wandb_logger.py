...
class WandbLogger:
    ...
    def run_url(self) -> str:
        ...
        return 'https://app.wandb.ai/{}/{}/runs/{}'.format(
            wandb.run.entity, wandb.run._project, wandb.run.id) if self._experiment else None
    ...
    def log_depth(self, *args, **kwargs):
        ...
        def log(prefix_idx, batch, output):
            images = []
            images.append(log_rgb('rgb', prefix_idx, batch))
            # images.append(slic_rgb('slic', prefix_idx, batch))
            images.append(log_inv_depth('inv_depth', prefix_idx, output))
            if 'depth' in batch:
                depth = batch['depth'] if is_dict(batch) else batch
                inv_depth = 1. / depth[0]
                inv_depth[depth[0] == 0] = 0
                inv_depth = inv_depth[0].cpu().detach().numpy()
                images.append(wandb.Image(get_colormap(inv_depth), caption='Ddepth'))
                images.append(log_depth('depth', prefix_idx, batch))
            prefix_key = '{}-{}'.format(prefix_idx, 'rgb-inv_depth-depth-Ddepth')
            self._metrics.update({prefix_key: images})
...
def slic_rgb(key, prefix, batch, i=0):
    """
    ...
    """
    segements = batch[key] if is_dict(batch) else batch
    image = batch['rgb'][i].permute(1, 2, 0)
    markBoundaries = mark_boundaries(image.cpu().detach().numpy(), segements[i].cpu().detach().numpy())
...
def prep_image(prefix, key, image):
    ...
    """
    if is_tensor(image):
        image = image.detach().permute(1, 2, 0).cpu().numpy() # [H, W, 3]
    prefix_key = '{}-{}'.format(prefix, key)
    # return {prefix_key: wandb.Image(image, caption=key)}
    return wandb.Image(image, caption=key)
